...
function buildGallery(images) {
  shuffle(images);
  const cols = Math.floor(Math.sqrt(images.length));
  const columns = [];
  panzoomEl.innerHTML = '';
  ...
  images.forEach(src => {
    ...
    img.onload = () => {
      ...
      // metti nella colonna più corta
      const short = columns.reduce((a,b) =>
        a.offsetHeight < b.offsetHeight ? a : b
      );
      short.appendChild(tile);

      // quando l’ultima è caricata, centro
...
// — pan & pinch solo su desktop (non-touch)
